...
function setupMapInteraction(year) {
    ...
    // Touch events için
    newMapImage.addEventListener('touchstart', function(e) {
    ...
    newMapImage.addEventListener('touchmove', function(e) {
        if (e.touches.length === 2) {
            e.preventDefault();
    ...
    newMapImage.addEventListener('touchend', function() {
        if (this.style.transform) {
            currentScale = parseFloat(this.style.transform.replace('scale(', '').replace(')', '')) || 1;
        }
    });

    // Mouse click için
    newMapImage.addEventListener('click', function(e) {
        if (currentScale === 1) {  // Sadece zoom yapılmamışsa tıklamaya izin ver
